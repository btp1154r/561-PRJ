...
#define	CS_HIGH()	{P3 |= 1;}	/* Set CS High */
#define	CS_LOW()	{P3 &= ~1;}	/* Set CS Low */
#define	INS	        (1)	/* Card status switch (Exist:True, Empty:False) */
#define	WP		0 						/* Write protect switch (Protected:True, Enabled:False) */

#define CTRL_INIT(){  \
        PM3 &= (UCHAR)~1;  /* P30 output */  \
        P3 |= (UCHAR)1;  /* P30 SD-CS high. */ \
}
...
#define SD_Send(buffer,length) SPI_Send(1, buffer,length) /* Send data to SD SPI */
#define SD_Receive(buffer,length) SPI_Receive(1, buffer,length) /* Send data to SD SPI */
#define SD_SendReceive(txBuffer, txLength, rxBuffer) SPI_SendReceive(1, txBuffer, txLength, rxBuffer) /* Send and Receive data to/from SD SPI */
...
)
{
	...
	r = xchg_spi(0xFF);		/* Read a byte (Force enable DO output) */
	do {
                r = xchg_spi(0xFF);
                if (r == 0xFF)
                {
